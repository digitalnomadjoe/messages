...
# Fixed paths the Custom GPT Action reads. Every one is a constant so the
# action's OpenAPI schema needs no free-form path parameter at all.
TELEPHONE_INSTRUCTIONS_PATH = f"{ml.DIR_STATE}/telephone_instructions.md"
BROWSER_RESULTS_DIR = f"{ml.DIR_STATE}/browser_results"
INSTRUCTION_SOURCES = ("AGENTS.md", "TELEPHONE.md", "skills/telephone/SKILL.md")
RECENT_RESULTS_WINDOW = 25
...
class BrowserBridge:
    ...
    def write_action_artifacts(self, msgs: dict) -> list[str]:
        """Fixed-path files the Custom GPT Action reads. Returns changed paths."""
        changed: list[str] = []

        instructions = build_instructions(self.repo)
        ipath = self.repo.path / TELEPHONE_INSTRUCTIONS_PATH
        ipath.parent.mkdir(parents=True, exist_ok=True)
        if not ipath.exists() or ipath.read_text(encoding="utf-8") != instructions:
            ipath.write_text(instructions, encoding="utf-8")
            changed.append(TELEPHONE_INSTRUCTIONS_PATH)

        rdir = self.repo.path / BROWSER_RESULTS_DIR
        rdir.mkdir(parents=True, exist_ok=True)
        for rid, receipt in self.answered(msgs).items():
            rec = result_record(rid, receipt, msgs)
            text = json.dumps(rec, indent=2) + "\n"
            if ml.scan_secrets(text, self.private):
                raise MessageError(f"result record for {rid} failed the secret scan")
            rel = f"{BROWSER_RESULTS_DIR}/{rid}.json"
            path = self.repo.path / rel
    ...
    def write_status(self, msgs: dict) -> None:
        extra = self.write_action_artifacts(msgs)
...
def result_record(rid: str, receipt, msgs: dict) -> dict:
    ...
    Carries IDs and a short reason only -- never request prose, ticket bodies,
    local paths or evidence.
    """
    accepted = receipt.get("status") != "blocked"
    produced: list[str] = []
    if accepted:
        for m in sorted(msgs.values(), key=ml.Message.sort_key):
            if m.get("request_id") == rid and m.id != receipt.id:
                produced.append(m.id)
    rec = {
        "request_id": rid,
        "status": "accepted" if accepted else "refused",
        "result_receipt": receipt.id,
        "canonical_message_ids": produced,
        "decided_at": str(receipt.get("created_at")),
    }
    if not accepted:
        rec["reason"] = str(receipt.get("reason") or "")[:200]
    return rec


def build_instructions(repo: ml.Repo) -> str:
    """One file combining the canonical guides, so the Action reads a fixed path."""
    parts = [
...
def build_browser_status(cfg: dict, msgs: dict, repo: ml.Repo,
    ...
    if bridge:
        ...
        for rid, receipt in sorted(answered.items()):
            recent.append(result_record(rid, receipt, msgs))
        recent = sorted(recent, key=lambda x: x["request_id"])[-RECENT_RESULTS_WINDOW:]
